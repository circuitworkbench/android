...
Tap the 'Run Simulation' button in the editor toolbar. The Simulation Wizard opens.
...
3. Output Vectors: Select which node voltages, branch currents, and device parameters to retrieve. The default set includes all node voltages and voltage source currents. If meters are used in the circuit, the defaults are vectors from the meters. Add custom vectors by typing them in — use standard SPICE vector syntax (v(nodename), i(vsource), @device[param]).

Tap Run on the final page to execute the simulation. The console tab opens and displays messages from the SPICE engine while the simulation runs.

RESULTS
For OP, values are shown as overlays on the canvas. The results are shown close to the components they are associated with. For DC, AC and Transient, values are shown as waveforms in the waveform viewer.
